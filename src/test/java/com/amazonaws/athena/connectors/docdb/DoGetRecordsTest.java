...
public class DoGetRecordsTest extends RealMongoTest implements AthenaTest {
    ...
    @SuppressWarnings("deprecation")
    @Test
    public void doTestField() {
        try (MongoClient mongoClient = MongoClients.create(mongoDBContainer.getConnectionString())) {

            mongoClient.getDatabase("bravo").getCollection("foo_1").insertOne(new Document());
            mongoClient.getDatabase("bravo").getCollection("foo_2").insertOne(new Document());

            Schema schema = SchemaBuilder.newBuilder()
                    .addField("myid", MinorType.VARCHAR.getType())
                    .build();
            ...
            BlockSpiller blockSpiller = new InMemorySingleBlockSpiller(schema, ConstraintEvaluator.emptyEvaluator());
            ReadRecordsRequest request = new ReadRecordsRequest(getIdentity(), "missing", generateId(), new TableName("bravo", "foo_myid"), schema, split, new Constraints(Collections.emptyMap()), Integer.MAX_VALUE, Integer.MAX_VALUE);
            getRecords.readWithConstraint(blockSpiller, request, () -> true);
            System.out.println(blockSpiller.getBlock().getRowCount());
        }
    }
